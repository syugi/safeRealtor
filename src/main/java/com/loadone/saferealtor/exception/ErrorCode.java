...
public enum ErrorCode {
    ...
    INVALID_VERIFICATION_CODE("인증번호가 일치하지 않습니다.", HttpStatus.UNAUTHORIZED),
    EXPIRED_VERIFICATION_CODE("인증번호가 만료되었습니다.", HttpStatus.UNAUTHORIZED),
    FAILED_TO_SEND_VERIFICATION_CODE("인증번호 발송에 실패하였습니다."),
    FAILED_TO_REGISTER_USER("사용자 등록에 실패하였습니다."),

    //사용자
    INVALID_USER_ID("사용 불가능한 아이디 입니다."),
    DUPLICATED_USER_ID("이미 사용중인 아이디입니다.", HttpStatus.CONFLICT),
    DUPLICATED_PHONE_NUMBER("이미 회원가입된 전화번호입니다.", HttpStatus.CONFLICT),
